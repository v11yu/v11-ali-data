 ...
 */
public class OptionTree extends JPanel {
	...
	/**
	 * the text field for options represented by the currently selected
	 * sub-tree.
	 */
	...
	protected void initGUI() {
		...
		m_TextOptionsFull.getDocument().addDocumentListener(
		...
		m_TreeOptions.getSelectionModel().addTreeSelectionListener(
		...
		m_TextOptionsSelected.getDocument().addDocumentListener(
				new DocumentListener() {
					public void changedUpdate(DocumentEvent e) {
						update();
					}
	...
	/**
	 * Parses the given option and adds the generated parts to the parent. If
	 * there are nested options, then the parsing follows depth-first.
	 * 
	 * @param parent
	...
	protected String getOptions(DefaultMutableTreeNode node) {
		...
		// the node itself
		if (!node.toString().equals(LABEL_ROOT)
				&& !node.toString().equals(LABEL_NESTED))
	...
	protected void generatePartialOptions() {
		...
		node = (DefaultMutableTreeNode) m_TreeOptions.getSelectionPath()
